...
export default function RegisterUserForm({ onRegistered }: RegisterUserFormProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      // Create a new keypair for the user account (since it needs to be a signer)
      ...
      // Call the registerUser instruction with the user account as signer
      const tx = await program.methods
        .registerUser(roleEnum, name, matricNumberValue)
        .accounts({
          user: userAccount.publicKey,  // Use the keypair's public key
          authority: publicKey,  
          systemProgram: SystemProgram.programId,
        })
        .signers([userAccount])  // Add the user account as an additional signer
        .rpc();
